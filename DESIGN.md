...
- メッセージ右上の時刻はブラウザのタイムゾーンで年月日・24時間制の時分を
  日本語形式で表示する。標準 Date / Intl で元の Z / offset を解釈し、日付境界と
  夏時間を扱う。保存値・API・`time` の `datetime` は原値を保ち、欠損または
  Date が解釈できない値では時刻を表示しない。
...
- 詳細 chrome 直下に高さ 44px の操作バーを確保し、本文の scroll から独立させる。
  ...
  タブ横の常設説明は置かない。状態と「最新へ戻る」quiet-button はこのバーに置き、
  状態のないときは空の live region を保つ。長い状態・エラー文は高さ 44px 以内の内部 scroll で
