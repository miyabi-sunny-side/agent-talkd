...
4. `turn-end`は宛先をidleにし、queue先頭を1件だけ配送してbusyへ戻します。
   ...
   ためです。配送の安全境界（busyへ送らない、herdrのidleガード）は不変で、
   検出が追いついた次のtickで同じIDがFIFOのまま流れます。
...
8. 配達済みのまま受領報告が1分間ないメッセージには、宛先がidleのときだけ
   受領催促の呼び鈴を送ります。読了済みならack、未読なら読むことを促し、
...
idleのときだけdaemonが受領催促の呼び鈴を送ります。読了済みならack、未読なら読む
ことを促し、pane単位で1回の呼び鈴に集約し、同じメッセージへは5分間隔より詰めま
せん。busy中は撃たず、催促は新しいメッセージを作りません（催促自体が受領報告の
